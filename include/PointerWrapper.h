...
public:
     ...
     */
    PointerWrapper(PointerWrapper&& other) noexcept : ptr(other.ptr) {other.ptr = nullptr;}
    ...
    T* operator->() const {
        if (!ptr) {
        throw std::runtime_error("Attempted to use arrow operator on null PointerWrapper");
        }
        return ptr;
    }
    ...
    T* get() const {
        if (!ptr) {
            throw std::runtime_error("Attempted to get null pointer from PointerWrapper");
        }
        return ptr;
    }
